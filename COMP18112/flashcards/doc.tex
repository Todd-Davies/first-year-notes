...
\card{
	...
	When a host moves location in the network, we shouldn't need to the details
	of the move.\\
...
\flashcard{
	...
	distributed}, and could be refered to as a form of \blank{divide-and-
	conoquer} strategy. However, if you have a network built of heterogenous
...
\flashcard{
	...
	\blank{buisiness to buisiness integration} (B2B) system.
}
...
\card{
	...
	then sent to a reciever. The reciever will then respond via the same method
...
\card{
	...
	\begin{tabularx}{0.5\textwidth}{lX}
		- & Security, authemtication and authorization\\
...
\card{
	...
	\includegraphics[scale=0.4]{images/rest-vs-soap}
}
...
\card{
	...
	data sources. It does this by constructing many smaller queries on it's
...
\flashcard{
	...
	cache and returns it if so. Otherwise, it will \blank{propogate the request
	up} the heirachy until it does find an answer.
}
...
\card{
	...
	heirachy. TTL values can be from seconds up to 68 years, or never.
}
...
\card{
	...
	any changes that have occured. It also lets users have multiple inboxes,
...
\card{
	...
	\begin{tabularx}{0.5\textwidth}{>{\bfseries}l X}
		Mutual Exclusion & There must be at least one resource that cannot be
						   shared.\\
		Hold and Wait &    A process is currently holding on (or more)
		...
		Circular wait &    There must be a cycle of depedencies so that
...
\card{
	...
	\begin{tabularx}{0.5\textwidth}{lX}
		- & Assume that a succesful coordinated action occurs as a result of
...
\card{
	...
	Napster had a centeral server which acted as a file directory/search engine
...
\card{
	...
	\begin{tabularx}{0.5\textwidth}{lX}
		...
		- & Nodes would be promoted to supernodes if they had sufficient uptime
		    and bandwitdh (though they could demote themselves if they didn't
...
\card{
	In a disitributed hash table, how many keys is each node responsible for if
...
\flashcard{
	...
	finger table constains the address of the successor
	$(n + 2^{i-1} ) mod 2^m$.
}
...
\card{
	...
	also meant there was no saved state if all the players discontected.
}
...
\card{
	...
	clients left. Client side prediction was utalised to reduce network load.
}
...
\card{
	...
	Describe, explain, jusify, contrast
}
