...
A computer can make decisions, and choose which instructions to obey next depending upon the results of those decisions. A {\bf Program Counter} (PC) register is used to hold the memory address of the next instruction to be excecuted. ARM uses register 15 as its PC.

\subsection{Fetch-Execute Cycle}

The processor must first fetch instructions from memory before it can excecute them. This is called the fetch-execute cycle, and it involves:
...
In order to make decisions, the computer musn't just execute instructions one after the other in a linear manner. Instead, branches must be used to change the sequence of instructions to be excecuted.
