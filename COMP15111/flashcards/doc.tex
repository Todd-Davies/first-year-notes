...
\card{
	...
	(this is a good way since you can pass lots of paramaters like this.)
}

\card{
	How should a method access stacked parameters?
}{
	Just read the stack using the {\tt STR} command and add an offset depending
...
\card{
	...
	A table of pointers to various methods for handeling different peripherals.
...
\card{
	...
	A hardware chip on the motherboard that allowd peripherals to write diretly
...
\card{
	...
	& & $\cdot$ & The motherboard will be more comlplicated with the extra
	hardware.
	\end{tabularx}
}
...
\card{
	...
	It also protects programs from eachother by letting them only access memory
...
\card{
	...
	A kernel mode conveys privaleges to a program. The default (user) mode only
	allows memory locations within the allocated space to be accessed. Other
	modes convey different privilages, e.g. the privelaged mode which allows any
