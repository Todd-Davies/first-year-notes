...
The memory usage of a running Java program can be broadly seperated into three
...
The stack area holds paramaters that are passed between methods, any local
...
Each object in the heap is composed of three parts; a header that contins
...
\subsection{Stale objects and gargage collection}
...
In Java programs, the repeated process of instantiateing a temporary object
that subsequently becomes obselete can result in the size of the heap growing
very quickly to unmanagable levels. The space taken up by referenced objects
...
\begin{enumerate}
	...
	the program and the heap don't change for the duration of the garbege
	collection.

	\item The GC (gargage collector) then walks through the heap from top to
...
\marginpar{This isn't the only way of implementing garbage collection. Other languages such as Python seek to avoid the 'stop-the-world' behaviour that the Java GC exhibits by using a technique called {\it reference counting} and reclaiming heap space as soon as objects are stale (having no references). The Java GC does aim to reduce the time it spends doing its job though; it has different levels of garbage collection major collections (that defragment and compact the heap) and minor collections (that only delete stale objects and then keep track of where the free space is for future allocation).}