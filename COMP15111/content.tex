...
addR1R2	ADD 	R1, R1, R2
	; Move the value of the Link Register back into the PC
	MOV 	PC, LR
...
\begin{listing}{1}
method 	; First lets store the registers we're going to use into memory
	PUSH 	{R0-R2}
	; ===========================
...
\begin{listing}{1}
; Define a table of variables with the locations to branch to
swtable	DEFW	case0
	DEFW 	case1
	DEFW 	default
	DEFW 	case2
; Load the base address of the table into R1
	ADR 	R1, swtable
; Load the value of R1 into the PC. Offset the value in R1 by (R0*4)
	LDR 	PC, [R1, R0, LSL, #2]
